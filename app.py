...
from utils.batch.mapping_batch import orderlines_mapping, locations_listing
from utils.cluster.mapping_cluster import df_mapping
from utils.batch.simulation_batch import simulation_wave, simulate_batch
from utils.cluster.simulation_cluster import loop_wave, simulation_cluster, create_dataframe, process_methods
...
st.set_page_config(
    page_title="Improve Warehouse Productivity using Order Batching",
...
with col_11:
    ...
    n2 = st.slider('SIMULATION 1: N_MAX (ORDERS/WAVE)', n1 + 1, 20, value=int(np.max([n1 + 1, 10])))
with col_22:
    st.write(f"[N_MIN, N_MAX] = [{n1:,}, {n2:,}]")

start_1 = st.checkbox('SIMULATION 1: START CALCULATION', key='show', value=False)
if start_1:
...
start_2 = st.checkbox('SIMULATION 2: START CALCULATION', key='show_2', value=False)
if start_2:
    with st.spinner("Running Simulation 2..."):
        df_orderlines = load('df_lines.csv', lines_2)
        df_reswave, df_results = simulation_cluster(
            y_low, y_high, df_orderlines, list_results, n1, n2, distance_threshold
        )
